  ...
  context 'without extra options' do
    ...
    describe 'env#relation' do
      it 'returns restricted and mapped object' do
        jane = container.relations[:users].map_with(:entity).by_name('Jane').to_a.first
      ...
      it 'returns specified attributes in mapped object' do
        jane = container.relations[:users].map_with(:entity).only_name.to_a.first
      ...
      it 'return ordered attributes by name and email' do
        results = container.relations[:users].map_with(:entity).ordered.to_a
      ...
      it 'returns joined data' do
        results = container.relations[:users].map_with(:entity_with_address)
    ...
    context 'with custom options' do
      ...
      it 'allows to force encoding' do
        user = container.relations[:users_utf8].to_a.last
